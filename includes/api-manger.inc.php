...
define("LB_TEXT_ACTIVATE_LIC", '<title>License not active</title>Your license is not active, please contact support<br>Have you tried to  <a href="../activate">activate it</a>?');
if(!LB_API_DEBUG){
...
class validationAPI{
    ...
    public function __construct(){
        $this->product_id = 'A8C099B2';
        $this->api_url = 'http://activation.daryan.technology/';
        $this->api_key = 'DB4A07C57705CA8D4E8E';
    ...
    public function validation($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["6a2f7d03be3ba0c"])){
                $_SESSION["6a2f7d03be3ba0c"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["6a2f7d03be3ba0c"])){
                $get_data = $this->call_api(
                    'POST',
